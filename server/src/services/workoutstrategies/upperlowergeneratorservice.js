...
export const generateUpperLower = async (user) => {
  ...
  const chest_push = workouts.filter(
    (w) =>
      w.muscles.includes("mell") &&
      w.name.toLowerCase().includes("nyomás") &&
      w.type != "cardio",
  );
  const chest_fly = workouts.filter(
    (w) =>
      w.muscles.includes("mell") &&
      w.name.toLowerCase().includes("tárogatás") &&
      w.type != "cardio",
  );
  ...
  const shoulders_push = workouts.filter(
    (w) => w.muscles.includes("elülsőváll") && w.type != "cardio",
  );
  const shoulders_delts = workouts.filter(
    (w) => w.muscles.includes("oldalsóváll") && w.type != "cardio",
  );
  ...
  let day = 1;
  const prototype_upper1 = () => {
    const rating = ratingGenerator(user);
    if (goal == "bulk") {
      if (rating <= 4) {
        exercises.chest_push = 1;
  ...
  const buildWorkout = () => {
    ...
    let pickedExercises = [];
    for (let i = 0; i < exercises.chest_push; i++) {
      const item = pickOne(chest_push, used, difficulty);
      if (item) {
        pickedExercises.push(item);
      }
    }
    for (let i = 0; i < exercises.chest_fly; i++) {
      const item = pickOne(chest_fly, used, difficulty);
      if (item) {
        pickedExercises.push(item);
      }
    }
    ...
    for (let i = 0; i < exercises.cardio; i++) {
      const item = pickOne(cardio, used, difficulty);
      if (item) {
        pickedExercises.push(item);
      }
    }
